...
/**
 * @file
 * just fetch lines from cache and let's see ??
 * 
 */
...
class Fetch1 : public Named
{
  protected:
    ...
    class IcachePort : public CClassCPU::CClassCPUPort
    {
      ...
      public:
        IcachePort(std::string name, Fetch1 &fetch_, CClassCPU &cpu) :
            CClassCPU::CClassCPUPort(name, cpu), fetch(fetch_)
        { }
  ...
  protected:
    ...
    struct Fetch1ThreadInfo
    {
         ...
         *  prediction sequence numbers. (my comment)that is if branchprediction or misprediction flush */
